...
const navLinks: NavLink[] = [
  {
    ...
    dropdown: {
      columns: [
        [
          { label: 'Sales Team Transformation Program', href: '/corporate-assessment' },
        ...
        [
          ...
          { label: 'Professional Sales Certification (EKPCS™)', href: '/register-cohort' },
    ...
    dropdown: {
      columns: [
        ...
        [
          ...
          { label: 'See All Industries', href: '#all-industries', highlight: true },
        ],
      ],
    },
  },
    ...
    dropdown: {
      columns: [
        ...
        [
          ...
          { label: 'See All Resources', href: '#all-resources', highlight: true },
        ],
      ],
    },
  },
...
const handleDropdownItemClick = (e: React.MouseEvent, href: string) => {
  ...
  return (
    <>
      <nav
        ref={navRef}
        className="fixed top-0 left-0 right-0 z-50 bg-black/95 backdrop-blur-md border-b border-[#222222] px-[5%] h-16 flex items-center justify-between"
      >
        {/* Logo */}
        <a
          href="/"
            ...
            <li key={link.href} className="relative">
              ...
              {link.dropdown && openIndex === index && (
                ...
                >
                  ...
                  <div className="p-6 grid gap-x-8"
                    style={{ gridTemplateColumns: `repeat(${link.dropdown.columns.length}, 1fr)` }}
                  >
                    {link.dropdown.columns.map((col, colIdx) => (
                      <ul key={colIdx} className="list-none flex flex-col gap-1">
                        {col.map((item) => (
                          <li key={item.href}>
                            <a
                              href={item.href}
        ...
        <button
          ...
          aria-label="Toggle menu"
        >
          <span className={`block w-6 h-0.5 bg-[#C8A96E] transition-all duration-300 ${mobileOpen ? 'rotate-45 translate-y-2' : ''}`} />
          <span className={`block w-6 h-0.5 bg-[#C8A96E] transition-all duration-300 ${mobileOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-6 h-0.5 bg-[#C8A96E] transition-all duration-300 ${mobileOpen ? '-rotate-45 -translate-y-2' : ''}`} />
        </button>
      </nav>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="fixed top-16 left-0 right-0 md:hidden bg-[#111111] border-b border-[#222222] z-40 animate-slideDown overflow-y-auto max-h-[calc(100vh-4rem)]">
      ...
      <style jsx>{`
        ...
        .animate-dropDown  { animation: dropDown  1.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
      `}</style>
    </>
  );
}
